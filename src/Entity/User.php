...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    #[ORM\OneToMany(mappedBy: 'user', targetEntity: ShopCard::class)]
    ...
    public function addshopCarts(ShopCart $shopCarts): self
    {
        if (!$this->shopCarts->contains($shopCarts)) {
            $this->shopCarts->add($shopCarts);
            $shopCarts->setUserId($this);
        }

        return $this;
    }

    public function removeshopCarts(ShopCart $shopCarts): self
    {
        if ($this->shopCarts->removeElement($shopCarts)) {
            // set the owning side to null (unless already changed)
            if ($shopCarts->getUserId() === $this) {
                $shopCarts->setUserId(null);
            }
        }
    ...
    public function removeShopCard(ShopCard $shopCard): self
    {
        if ($this->shopCards->removeElement($shopCard)) {
            // set the owning side to null (unless already changed)
            if ($shopCard->getUser() === $this) {
                $shopCard->setUser(null);
            }
        }
